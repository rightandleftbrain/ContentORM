    ...
    def create(cls, env):
        if cls.exist(env):
            raise OperationalError(
                'Content type ' + camel_case(cls.__name__) + ' already exist. Use update() to update fields.')
    ...
    def serialize(cls):
        ...
        for attr in _get_class_attr(cls):
            ...
            if inspect.getmro(type(obj))[-2] != fields.Field:
                raise TypeError(
                    'Model fields must be a Field based class. Field (' + attr + ') is ' + str(type(obj)) + '.')
            obj.set_name(attr)
        ...
        if attributes['displayField'] is None:
            raise ValueError('__display_field__ must be one of the attributes. `' + str(
                cls.__display_field__) + '` cannot be found.')
        return attributes
